...
function Shop() {
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredProduct = response.data.products.filter(product => product.status === 'ACTIVE');
  ...
  const fetchDataProduct = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/shoestore/api/products/getMostPurchased"
      );
  ...
  return (
    <>
        <div class="container transparent-background" style={{ marginTop: '20px', padding: '20px' }}>
          <TextField
          ...
          <div class="carousel flex ">
            <button onClick={handlePrev} class="carousel-button">&lt;</button>
